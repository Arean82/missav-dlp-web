...
- **📄 문서 뷰어:** 웹 인터페이스에서 로컬 README, SECURITY, LICENSE 파일을 직접 볼 수 있는 내장 모달.
- **🔗 커스텀 URL 크롤러:** 시리즈, 제작사 또는 검색 URL을 입력하고 필터를 선택하여 스크래핑할 페이지 수를 지정한 후 결과를 직접 다운로드.
- **📂 대화형 폴더 브라우저:** 웹 UI에서 다운로드 경로와 FFmpeg 디렉토리를 안전하게 찾아보고 선택 (Docker 및 포터블 호환).
...
| `/api/crawl/filters`       | POST     | URL에 대한 사용 가능한 필터 가져오기 |
| `/api/utils/ls`            | POST     | 서버 측 디렉토리를 안전하게 탐색 및 나열 |
...
- **지능형 회퇴**: 누락된 태그를 보완하기 위한 **멀티 사이트 회퇴** (BestJavPorn/JavGuru) 기능 추가.
- **모듈식 프록시**: 메타데이터 및 크롤링에 SpoofDPI 우회를 적용하는 글로벌 토글 추가.
- **산업용 런처**: Light/Dark 테마 지원 및 Windows 작업 표시줄 통합이 포함된 새로운 CustomTkinter UI.
- **경로 선택**: 안전하고 시각적인 경로 선택을 위한 **대화형 폴더 브라우저** 구현 (Docker 호환).
- **안정성**: 분리된 SpoofDPI 인스턴스 생성을 방지하도록 프로세스 라이프사이클 개선.
- **제어**: 메타데이터 및 크롤러 우회를 위한 **모듈식 글로벌 프록시** 토글 추가.
- **정밀도**: 수동 입력 없이 안전한 경로 선택을 위한 **대화형 폴더 브라우저** 추가.
