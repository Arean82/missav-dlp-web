...
- 📄 **ドキュメントビューア:** WebインターフェースからローカルのREADME、SECURITY、LICENSEファイルを直接表示。
- 🔗 **カスタムURLクローラー:** シリーズ、メーカー、または検索URLを入力し、フィルターを選択して、スクレイピングするページ数を指定。
- 📂 **インタラクティブフォルダブラウザ:** Web UIからダウンロードパスとFFmpegディレクトリを安全に参照して選択（Dockerおよびポータブル対応）。
...
| `/api/crawl/filters`       | POST     | URLの利用可能なフィルターを取得 |
| `/api/utils/ls`            | POST     | サーバー側のディレクトリを安全に参照および一覧表示 |
...
- **インテリジェント・フォールバック**: 欠落したタグを補完するための **マルチサイト・フォールバック** (BestJavPorn/JavGuru) 機能を追加。
- **モジュール式プロキシ**: メタデータとクローリングにSpoofDPIバイパスを適用するグローバル切り替えを追加。
- **産業用ランチャー**: Light/Darkテーマ対応およびWindowsタスクバー統合を備えた新しいCustomTkinter UI。
- **パス選択**: 安全で視覚的なパス選択のための **インタラクティブフォルダブラウザ** を実装（Docker対応）。
- **安定性**: 孤立した SpoofDPI インスタンスを防ぐためのプロセス管理を最適化。
